...
mas = DB(host='10.146.25.8',user=db_user ,passwd=db_pass)
...
def activate(argv1):
    try:
        ...
        check = mas.query("SELECT description, fqdn FROM prefixlist LEFT JOIN dns ON (prefixlist.primary_subnet = dns.primary_subnet) LEFT JOIN codes ON (prefixlist.prov_group_id = codes.code_id) WHERE '%s'  <<= address LIMIT 1" % (argv1))
    ...
    try:
        ...
        check=mas.query("SELECT description, fqdn FROM scopelist LEFT JOIN dns ON (scopelist.primary_subnet = dns.primary_subnet) LEFT JOIN codes ON (scopelist.prov_group_id = codes.code_id) WHERE subnet && '%s'" % (argv1))
        result = check.getresult()
        pg, fqdn = zip(*result)
        #print(pg[0])
        #print(fqdn[0])
